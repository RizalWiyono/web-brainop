...
<body>
        ...
        <div class="left-section" style="height: auto;">
            <div class="nav-dashboard">
                <div class="header">
                    ...
                    <h1 class="mt-2 mb-2">BrainOp CS</h1>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="place-chat">
                                <ul class="list-user-chat place-message-out">
                                    <?php
                                        $queryUserChat  = mysqli_query($connect, "SELECT chat.iddataUser, name, first_name, date FROM chat INNER JOIN datauser ON chat.iddataUser = datauser.iddataUser GROUP BY chat.iddataUser DESC");
                                        while($row = mysqli_fetch_array($queryUserChat)){ ?>
                                       <li class="mt-2">
                                            <form action="" method="get">
                                                <input type="hidden" name="id" value="<?=$row["iddataUser"]?>">
                                                <button style="background: transparent; border: 0;">
                                                    <h1>
                                                        <?=$row["first_name"]?>
                                                        <?php
                                                        $idData = $row["iddataUser"];
                                                        $dataCount  = mysqli_query($connect, "SELECT COUNT(*) FROM `chat` WHERE iddataUser=$idData && status='Unread' GROUP BY status;");
                                                        while($rowdata = mysqli_fetch_array($dataCount)){ 
                                                            if($rowdata['COUNT(*)'] > 0){ ?>
                                                                <span class="badge badge-success"><?=$rowdata['COUNT(*)']?></span>
                                                            <?php }else{ ?>
                                                            <?php }
                                                        } ?>
                                                    </h1>
                        ...
                        <div class="col-md-9">
                            <div class="place-chat">
                                ...
                                <hr>
                                <div class="place-message-out">
                                    <div>
                                    <?php
                                        $idUserChat = $_GET["id"];
                                        if($idUserChat){
                                            $queryChat  = mysqli_query($connect, "SELECT * FROM chat WHERE iddataUser=$idUserChat ORDER BY idchat ASC");
                                            while($row = mysqli_fetch_array($queryChat)){ ?>
                                            <?php 
                                                if($row["chatAdmin"] === ' '){ 
                                                    $haystack = $row["chatUser"];
                                                    $needle   = 'tagorder';
                                                    if (strpos($haystack, $needle) !== false) { 
                                                        $orderId = substr($haystack,9); ?>
                                                        <div style="display: flex;">
                                                            <div class="detail-order-chat-admin">
                                                                <?php
                                                                $queryDetailOrder  = mysqli_query($connect, "SELECT * FROM detailorder WHERE iddetailOrder='$orderId'");
                                                                while($row = mysqli_fetch_array($queryDetailOrder)){ ?>
                                                                    <div class="d-flex justify-content-between">
                                                                        <div class="d-flex">
                                                                            <h1 class="mr-2"><?=$row["project_name"]?></h1>
                                                                            <h1>(<?=$row["company_name"]?>)</h1>
                                                                        </div>
                                                                        <span class="badge badge-light"><?=$row["project_package"]?></span>
                                                                    </div>
                                                                    <p><?=$row["description"]?></p>
                                                                    <a href="../detail-order/?pr=new">
                                                                        <button class="btn btn-secondary" style="font-size: 12px; padding: 1px 10px !important;">Details</button>
                                                                    </a>
                                                                    <span style="font-size: 8px;"><?=$row["date"]?></span>
                                                                <?php } ?>
                                                            </div>
                                ...
                                <hr>
                                <div class="chat-area">
                                    <form action="fetch/inputData.php" method="post" class="chat-area" style="width: 100%;">
                                        <input type="hidden" name="iduser" value="<?=$_GET["id"]?>">
                                        <textarea name="chat" class="ckeditor" id="ckedtor"></textarea>
    ...
    <?php
    $idUserData = $_GET['id'];
    mysqli_query($connect, "UPDATE chat SET status='Read' WHERE iddataUser = $idUserData");
    ?>
</body>
